...
class ErisaAnalyzer:
    def __init__(self, input_dir, input_filename, file_type, theme):
    ...
    def generate_prompt_from_template(self, template_bdd="", output_filename="erisa_result.txt", output_dir="./output"):
        ...
        save_results(result_prompt, output_filename, "_result", output_dir)

    def execute_analysis(self):
        logger.info(f"Executing '{self.theme.capitalize()}' analysis...")
        ...
        if self.theme.lower() not in template_prompts:
            logger.error(f"Theme '{self.theme}' not found in template JSON. Available themes: {list(template_prompts.keys())}")
            sys.exit(1)

        self.generate_prompt_from_template(
            template_bdd=template_prompts[self.theme.lower()],
            output_filename=f"{self.input_filename}_{self.theme.lower()}.{self.file_type}"
        )
        logger.info(f"Analysis complete for theme: {self.theme.capitalize()}.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run ERISA Analysis for a Specific Theme")
    ...
    parser.add_argument("-t", "--theme", required=True, help="Theme for analysis")

    args = parser.parse_args()

    analyzer = ErisaAnalyzer(args.input_dir, args.input_filename, args.file_type, args.theme)
